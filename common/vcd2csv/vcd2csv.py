		...
		if len(split_line) > 1: # Get specified bit ranges for signal
			...
			for sig_range in split_line[1:]:
				...
				if sig_ranges[sig][-1][0] < sig_ranges[sig][-1][1]:
					print("ERROR: Illegal range for signal " + sig + ": [" + str(sig_ranges[sig][-1][0]) + ":" + str(sig_ranges[sig][-1][1]) + "]")
if not found_clock:
...
for sig in sigs[1:]:
	...
	if len(sigs_trace[sig]) == 0:
		sig_entry_new = bytearray(sig_data[-1][1])
		for char_idx in range(len(sig_entry_new)):
			if sig_entry_new[char_idx] == ord('x') or sig_entry_new[char_idx] == ord('z'):
				sig_entry_new[char_idx] = ord('0')
				xz_count += 1
		sigs_trace[sig].append(int(str(sig_entry), 2))
